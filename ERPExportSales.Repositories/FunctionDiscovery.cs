        ...
        private FunctionDescriptor CreateFunctionDescriptor(MethodInfo method)
        {
               ...
               returnGenericTypeDefinition == typeof (ObjectResult<>))  // StoredProc without DbFunction attribute
            {
                ...
                if (storeFunctionKind == StoreFunctionKind.ScalarUserDefinedFunction &&
                    (functionAttribute == null || functionAttribute.NamespaceName != "ERPExportSalesSchema"))
                {
                    throw new InvalidOperationException("Scalar store functions must be decorated with the 'DbFunction' attribute with the 'ERPExportSalesSchema' namespace.");
                }
